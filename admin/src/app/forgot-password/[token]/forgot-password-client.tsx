...
export default function ForgetPasswordPageClient({
  token,
}: {
  token: string;
}) {
  const [adminConfirmForgotPassword] = useAdminConfirmForgotPasswordMutation();
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const onSubmit = async (data: { password: string }) => {
    ...
    if ("error" in res) {
      ...
      if ("data" in res && res.data && "message" in res.data) {
        notifySuccess((res.data as { message: string }).message);
      }
      reset();
    }
  };
